...
__DEBUG__ = os.environ.get('STATICPLAYLISTPVRDEBUG')
app       = default_app()
port      = settings.port

@route('/staticplaylist.backend/playlist', method=GET)
def get_playlist():
  """
...
@route('/staticplaylist.backend/stream/<name>', method=HEAD)
def get_stream(name):
  return HTTPResponse(None, 
                      status=200)


@route('/staticplaylist.backend/stream/<name>', method=GET)
def get_stream(name):
  '''
